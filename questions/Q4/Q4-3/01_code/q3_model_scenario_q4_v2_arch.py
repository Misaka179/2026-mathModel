...
# Q4-3 v2架构修改（按GPT-RE2.md建议）：
# 1. 紧急购电从优化变量改为补偿项（derived variable）
# 2. 恢复使用波动电价：C_emg = 5 * price[t] * E_emg
# 3. 架构与Q4-2一致：第一阶段只优化E_plan，紧急购电在实际执行时计算
# BASE_PRICE_EMERGENCY 不再使用
...
def infer_day_types_from_jan1_14(load_by_date: Dict) -> set:
    ...
    """
    global LOW_LOAD_WEEKDAYS  # v2修复：设置全局变量
    ...
    # v2修复：设置全局变量
    LOW_LOAD_WEEKDAYS = low_load_weekdays
...
def select_representative_scenarios(target_date: datetime, load_by_date: Dict,
    ...
    """
    # v2修复：移除硬编码日期限制，动态确定数据起始日期
    # 找到数据集中的最早日期
    all_dates = sorted(load_by_date.keys())
    if not all_dates:
        return []
    ...
    for i in range(1, 31):
        check_date = target_date - timedelta(days=i)
        if check_date >= min_date and check_date in load_by_date and check_date in pv_by_date:
    ...
    if len(selected_dates) < n_scenarios:
        ...
        similar_candidates = []
        for i in range(31, 365):  # 从31天前到365天前（扩大范围）
            check_date = target_date - timedelta(days=i)
...
def solve_stage1_scenario(date: datetime, price: np.ndarray, scenarios: List[Dict],
    ...
    # 各情景的追索变量（v2架构修改：移除E_emergency_scenarios）
    E_charge_scenarios = []
    ...
    for omega in range(n_scenarios):
        # v2架构：不再将紧急购电作为优化变量
        # E_emergency_scenarios.append([...])  # 已移除
    ...
    for omega in range(n_scenarios):
        ...
        for t in range(N_SLOTS):
            ...
            # v2架构：电力平衡改为不等式，允许供电不足
            # 缺口将在实际执行时由紧急购电补偿（作为补偿项，不是优化变量）
            prob += (E_plan[t] + ETA * E_discharge_scenarios[omega][t] * DT >=
...
def execute_single_day(date: datetime, price: np.ndarray, load_real_day: np.ndarray,
    ...
    # Z_emergency: 紧急购电成本（Q4-3修改：使用固定基准价）
