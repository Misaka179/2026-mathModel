...
三人交接协议。核心：**标准化中间产物交接，所有文件push到仓库分支，负责部分全部完成后merge到main并及时在群内交接**。
...
群内交接：**「Q1 模型已冻结，B 可开始接手。commit: xxxxx」**，接收方 `git pull` 后开工。
...
| 9.12 20:00 | 完成所有模型求解结果并不再更改大方向 | 全队 |
